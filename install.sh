...
TEMP_SOURCE=""
if [[ ! -f "$SOURCE_DIR/package.json" && -f "$SOURCE_DIR/Painel DPTunnel/package.json" ]]; then
  SOURCE_DIR="$SOURCE_DIR/Painel DPTunnel"
fi
...
[Unit]
Description=Painel DPTunnel - ${BRAND}
...
desinstalar() {
  ...
  printf '%b\n' "${AMARELO}${BOLD}⚠ Desinstalação do Painel DPTunnel${RESET}"
...
[Unit]
Description=Bot Telegram de backup do Painel DPTunnel - ${BRAND}
...
[Unit]
Description=Backup automático do Painel DPTunnel - ${BRAND}
After=network-online.target paineldtmod.service
Wants=network-online.target
